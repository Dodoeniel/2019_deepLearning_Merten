...
X_ts = Data[0][0] # for window only Data[0]
labels_td = Data[0][1]
...
def getLengthOfClassified(ClassifiedDataSet, eec):
    if not ClassifiedDataSet.empty:
        timeList = list()
        for id in ClassifiedDataSet['stopId'].unique():
            timeList.append(eec.loc[eec['stopId'] == id, 'stoptime'])
        return timeList
    else:
        timeList = list()
        return timeList
...
for i in range(max(len(FN_time), len(TN_time), len(TP_time), len(FP_time))):
    ...
    FP_one = ''
    if i < len(FN_time):
        FN_one = FN_time[i].values[0]
    if i < len(TN_time):
        TN_one = TN_time[i].values[0]
    if i < len(TP_time):
        TP_one = TP_time[i].values[0]
    if i < len(FP_time):
        FP_one = FP_time[i].values[0]
    writer.writerow([FP_one, FN_one, TP_one, TN_one])
